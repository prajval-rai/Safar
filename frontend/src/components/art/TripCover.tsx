 ...
 * Destination artwork.
 *
 * Every trip and track gets an illustrated cover drawn as inline SVG: it loads
 * instantly, never 404s, scales to any card size and costs no bandwidth. If a
 * trip has a real photo in `cover_image`, that wins and the drawing is the
 * fallback behind it.
 */
...
function Scene({ cover }: { cover: CoverKey }) {
  ...
  return (
    <>
      ...
      <rect width="400" height="200" fill={`url(#sky-${cover})`} />
    ...
    case "backwater":
      return (
        <>
          ...
          />
          {cover === "backwater" ? (
            <path d="M212 168 h72 l-10 14 h-52Z" fill="#78350f" />
          ) : null}
        </>
      );
    case "mountain":
      ...
      return (
        <>
          ...
          {cover === "forest"
            ...
            : [
                "M20 152 Q70 142 120 152",
                "M140 162 Q196 150 252 162",
                "M262 148 Q318 138 374 148",
              ].map((d) => (
    ...
    default:
      return (
        <>
          ...
          <path d="M138 200 Q184 152 172 118 Q166 96 200 84 Q238 72 250 96" stroke={p.near} strokeWidth="26" fill="none" strokeLinecap="round" />
          <path
...
export function TripCover({
  ...
  return (
    <div
      className={cn(
        "relative isolate overflow-hidden bg-raised",
        rounded && "rounded-2xl",
        className,
      )}
    >
      {image ? (
        ...
        // lazy <img> instead of next/image with a wide-open remotePatterns.
